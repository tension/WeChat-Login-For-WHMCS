...
if (!class_exists('WeLogin')) {
	class WeLogin {
		
		public function __construct(){
			
			$SystemURL = \WHMCS\Config\Setting::getValue('SystemURL');
			$SystemURL = $SystemURL . '/modules/addons/WeChat_Login/oauth/callback.php';
			
			
			$appid 			= Capsule::table('tbladdonmodules')->where('module', 'WeChat_Login')->where('setting', 'appkey')->first();
			$appsecret 		= Capsule::table('tbladdonmodules')->where('module', 'WeChat_Login')->where('setting', 'appsecret')->first();
	    ...
	    public function wechat_login(){
	        ...
	        $callback = \WHMCS\Config\Setting::getValue('SystemURL') . '/modules/addons/WeChat_Login/oauth/callback.php';
	        $scope = 'snsapi_login';
	        
			$login_url = "https://open.weixin.qq.com/connect/qrconnect?appid=".$appid."&redirect_uri=".$callback."&response_type=code&scope=".$scope."&state=1&connect_redirect=1&href=".\WHMCS\Config\Setting::getValue('SystemURL')."/modules/addons/WeChat_Login/weixin.css?v9#wechat_redirect";
	
	        header("Location: $login_url");
	    }
	    
	    public function WeMessage($status = '', $content = '', $extended = true) {
			// 获取 网站地址
			$SystemURL = \WHMCS\Config\Setting::getValue('SystemURL');
